class InterestSwap < ActiveRecord::Base
  ...
  scope :irfixedfloat, lambda {{:conditions =>
    ['taxonomy = ? AND execution_timestamp > ? AND action = ? AND settlement_currency = ?  AND price_notation < ?AND indication_of_other_price_affecting_term = ?', 'InterestRate:IRSwap:FixedFloat', (Time.now.to_i - 345600), 'NEW', 'USD', 100, 'N']}}
